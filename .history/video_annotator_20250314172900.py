...
class VideoAnnotator:
    ...
    def draw_emotion_labels(self, frame):
        """在视频左侧绘制情绪标签"""
        x, y = 20, 40  # **左上角起始位置**
        box_width, box_height = 200, 35  # **标签大小**
        ...
        # **加载更好的字体**
        try:
            font = ImageFont.truetype("Arial.ttf", 20)  # **确保 Arial 字体存在**
        except:
            font = ImageFont.load_default()

        for i, label in enumerate(self.emotion_labels):
            top_left = (x, y + i * (box_height + 5))  # **起点**
            bottom_right = (x + box_width, y + (i + 1) * box_height)  # **终点**
            
            # **绘制背景**
            draw.rectangle([top_left, bottom_right], fill=(150, 150, 150, 180))  # **半透明背景**
            
            # **绘制文本**
            draw.text((x + 10, y + i * (box_height + 5) + 8), f"{i} - {label}", font=font, fill=(255, 255, 255, 255))
