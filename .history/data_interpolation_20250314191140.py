...
def interpolate_csv(input_csv, output_csv, target_interval=1, visualize=False, method="linear"):
    ...
    if visualize:
        ...
        plt.plot(df_resampled["timestamp"], df_resampled["arousal"], 'b.-', label=f"Interpolated Data (1s, {method})", markersize=3)
        
        # **Update X-axis to match CSV first column labels**
        plt.xticks(df_resampled["timestamp"], labels=df_resampled["timestamp"].astype(str), rotation=45)
...
def visualize_interpolation(original_csv, interpolated_csv):
    ...
    plt.scatter(df_original["timestamp"], df_original["arousal"], color="red", label="Original Data (3s)", s=50, zorder=3)

    # **Plot interpolated data**
    plt.plot(df_interpolated["timestamp"], df_interpolated["arousal"], 'b-', label="Interpolated Data (1s, spline)", linewidth=1, zorder=2)
