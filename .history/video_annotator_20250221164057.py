...
class VideoAnnotator:
    ...
    def start_annotation(self):
        ...
        while True:
            ...
            # **启动音频，限制播放时间**
            audio_process = subprocess.Popen(["ffplay", "-nodisp", "-autoexit", "-t", str(self.segment_duration), self.audio_path],
                                             stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

            # **同步播放视频**
            video_start_time = time.time()  # 记录起始时间
            while self.cap.get(cv2.CAP_PROP_POS_MSEC) / 1000 < segment_end_time:
                ...
                elapsed_time = time.time() - video_start_time  # 计算已播放时间
...
if __name__ == "__main__":
    ...
    annotator = VideoAnnotator(video_path, output_file="annotations.json", segment_duration=5)
