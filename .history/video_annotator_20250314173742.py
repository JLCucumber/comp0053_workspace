...
class VideoAnnotator:
    ...
    def draw_emotion_labels(self, frame):
        """在视频左侧绘制情绪标签，使用统一的背景"""
        x, y = 20, 40  # **左上角起始位置**
        box_width, box_height = 220, 50  # **标签大小**
        spacing = 8  # **标签间距**
        ...
        draw.rectangle([background_top_left, background_bottom_right], fill=(50, 50, 50, 200))  # **半透明背景**

        # **加载支持 Emoji 的字体**
        try:
            font = ImageFont.truetype("NotoColorEmoji.ttf", 28)  # **使用支持 Emoji 的字体**
        except:
            font = ImageFont.truetype("Arial.ttf", 28)  # **Arial 备用字体**

        for i, label in enumerate(self.emotion_labels):
            text_position = (x + 10, y + i * (box_height + spacing) + 8)
